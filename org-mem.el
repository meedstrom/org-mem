...
;; Package-Requires: ((emacs "29.1") (el-job "2.7.1") (llama "0.5.0") (truename-cache "0.1.0"))
...
If Org has not yet loaded, only `recentf-list' is used."
  :type 'boolean
  :package-version '(org-mem . "0.31.0"))
...
Specifically, return the name by which it is known.  This is
more restrictive than `truename-cache-get-cached-p' by only returning
non-nil if the file has been scanned by org-mem."
  (cl-assert (stringp file))
  (or (and (gethash file org-mem--truename<>entries) file)
      (and (setq file (truename-cache-get-exists-p file))
   ...
   in (truename-cache-collect-files-and-attributes
       ...
       (append
        (and org-mem-do-look-everywhere
             (append (seq-filter (##string-match-p suffix-re %)
