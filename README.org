...
If you want to force-update the cache, you should now (since 0.28.0) be able to do it one of two ways.
...
The former cancels on ~C-g~, the latter continues to completion even if ~C-g~. Should usually not be an important distinction.

A full reset is very similar:
