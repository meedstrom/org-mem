...
And let's not talk about the org-agenda (try putting 2000 files into =org-agenda-files=).  It needs to open each file in real-time to know anything about them, so it grinds to a halt.
