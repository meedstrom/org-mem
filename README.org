...
For ending your dependence on =org-roam-db-sync=, set the following.
...
- indexed-id-nearby
  - (old alias: =indexed-origin=.  Org-node used "origin", org-roam uses "source", but both terms presume an ID-centric design to everything, and make less sense when you allow for the absence of IDs.)
