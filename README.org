...
** Limitation: =.org_archive=

Files ending in =.org_archive= are excluded from org-mem's database.  Or rather, it's more correct to say that it only includes =.org= files.

It is possible that IDs here get reported as "dead" IDs.

*TBD:* test that.  Easily fixable if so.
