...
This tells org-id about everything that Indexed can find under =indexed-org-dirs=.  If you had clicked an ID-link that org-id did not know about, it would react by running =org-id-update-id-locations=, making Emacs appear to hang for quite a while.
