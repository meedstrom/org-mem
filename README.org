...
#+HTML: <a href="https://repology.org/project/emacs%3Aorg-mem/versions"> <img src="https://repology.org/badge/vertical-allrepos/emacs%3Aorg-mem.svg" alt="Packaging status" align="right"> </a>
...
If you want to force-update the cache in synchronous code, you should now (since version 0.28.0) be able to do it one of two ways.
...
(org-mem-await "Manually updating..." 60)
#+END_SRC

The former cancels on ~C-g~, the latter continues the update in the background even if ~C-g~.  Should usually not be an important distinction, so pick the former because it's shorter.
...
*** Full list of functions [2026-02-02]
...
Getters operating on a file name /or/ entry object
...
Getters operating on a file name only

- =org-mem-entries-in-file= --- Alias =org-mem-file-entries=
- =org-mem-entries-in-files=
- =org-mem-entry-at-lnum-in-file= --- Semi-alias =org-mem-entry-at-file-lnum=
- =org-mem-entry-at-pos-in-file= --- Semi-alias =org-mem-entry-at-file-pos=
...
(However, org-mem is also careful not to scrub them from your =org-id-locations=, even if =org-mem-do-sync-with-org-id= is t, so your ID-links should still work.)
